...
namespace Bent.Bot.Module
{
    ...
    public class Wikipedia : IModule
    {
        ...
        public void OnStart(IConfiguration config, IBackend backend)
        {
            ...
            if (!String.IsNullOrEmpty(config[Bent.Bot.Common.Constants.ConfigKey.WikipediaAlias]))
            {
                this.regexAlias = new Regex(String.Format(@"^\s*{0},?\s+(.+?)\s*$", config[Bent.Bot.Common.Constants.ConfigKey.WikipediaAlias]), RegexOptions.IgnoreCase);
            }
        }
